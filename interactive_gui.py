...
from util.tensor_util import pad_divide_by, compute_tensor_iou,compute_np_iou
...
class App(QWidget):
    def __init__(self, prop_net, fuse_net, s2m_ctrl:S2MController, fbrs_ctrl:FBRSController, 
                    images, masks, num_objects, mem_freq, mem_profile,path_masks):
        super().__init__()

        self.images = images
        self.path_masks = path_masks 
        self.images_pad = images_pads(self.images)
        self.masks = masks
        ...
        # set window
        self.setWindowTitle('Faster-MiVOS')
    ...
    def console_push_text(self, text):
        ...
        # f = open(self.path_masks +"/kq_user5.txt", "a")
        # f.write(text)
        # f.write('\n')
        # f.close()
    ...
    def on_run(self):
        ...
        try:
            MIOU = 0
            for fr in range(len(self.current_mask)):
                MIOU += compute_np_iou(self.current_mask[fr],self.masks[fr])
            print(MIOU/len(self.current_mask))
            # f = open(self.path_masks +"/kq_user5.txt", "a")
            # f.write(str(MIOU/len(self.current_mask)))
            # f.write('\n')
            # f.close()
    ...
    def on_release(self, event):
        ...
        self.user_timer.start()
        # try:
        #     print(compute_tensor_iou(self.interacted_mask, self.masks[self.cursur],self.processor.pad)) 
        # except Exception as e:
        #     print(e)
        #     pass
        #print(self.interacted_mask.cpu().numpy()[0, 0])
        #print(self.masks[self.cursur])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prop_model', default='saves/STCN_retrain.pth')
    parser.add_argument('--fusion_model', default='saves/fusion.pth')
    parser.add_argument('--s2m_model', default='saves/s2m.pth')
    parser.add_argument('--fbrs_model', default='saves/ritm.pth')
    ...
    with torch.cuda.amp.autocast(enabled=not args.no_amp):
        ...
        ex = App(prop_model, fusion_model, s2m_controller, fbrs_controller, 
                    images, masks, num_objects, args.mem_freq, args.mem_profile,args.masks)
        sys.exit(app.exec_())